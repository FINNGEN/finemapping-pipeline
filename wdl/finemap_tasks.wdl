task preprocess {
    ...
    command {
        ...
        make_finemap_inputs.py \
            ...
            --exclude-MHC \
            --no-upload \
            --prefix ${true=prefix false=pheno defined(prefix)} \
            --out ${pheno} \
            --wdl \
...
task finemap {
    ...
    command <<<
        ...
        then
            ld_type="ld"
            ld_path="$(basename ${ld_bgz} .bgz)"
            zcat ${ld_bgz} > $ld_path
        else
            echo "bcor or ld_bgz should be provided." > /dev/stderr
            exit 1
        fi

        awk -v n_samples=${n_samples} -v ld_type=$ld_type -v ld_path=$ld_path '
        BEGIN {
            OFS = ";"
            print "z", ld_type, "snp", "config", "cred", "n_samples", "log"
            print "${zfile}", ld_path, "${prefix}.snp", "${prefix}.config", "${prefix}.cred", n_samples, "${prefix}.log"
        }' > ${master}

        prior_std=$(awk -v var_y=${var_y} 'BEGIN{print 0.05 * sqrt(var_y)}')
...
task filter_and_summarize {
    ...
    command <<<
        python ${script} --min_r2 ${good_cred_r2} ${susie_cred} \
